...
FGP_List(FilePath) {
	...
	for PropNum, PropName in PropTable.Num	
		if(val := oFolder.GetDetailsOf(oFolderItem, PropNum)) {
			props.DefineProp(PropName, {value:val})
			; propList.Push({num:PropNum, name:PropName, val:oFolder.GetDetailsOf(oFolderItem, PropNum)})	
		}
		; props := {num:PropNum, name:PropName, val:""}
